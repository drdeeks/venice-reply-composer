...
export class SelfVerificationService {
  ...
  constructor() {
    ...
    this.selfSDK = new SelfSDK({
      appId: config.selfAppId || 'dev-app',
      appSecret: config.selfAppSecret || 'dev-secret',
  ...
  async generateVerificationRequest(
    ...
    try {
      ...
      const request = await this.selfSDK.createVerificationRequest({
        email,
        callbackUrl: callbackUrl || `${config.baseUrl || 'http://localhost:3000'}/api/verifications/callback`,
